...
module.exports = async (data, hexo, option, PLUGIN_NM) => {
  let tags = data.content.match(tagMatchRegExp);
  if (tags) {
    // Apply replacements until there's no more md tag
    let tries = 0;
    do {
      tries++;
      if (tries >= MAX_RECURSIONS) {
        throw new Error (`[${PLUGIN_NM}] Too many circular inclusions.`);
      }
      await replace(data, option, PLUGIN_NM);
    } while (data.content.match(tagMatchRegExp));
